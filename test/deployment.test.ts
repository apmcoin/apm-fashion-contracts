...
const config = () => ({ bsc: { deployer: DEPLOYER, recipient: RECIPIENT }, bscTestnet: { deployer: null, recipient: null } });
...
describe("deployment", () => {
  it("requires configured, distinct nonzero addresses and a supported network", () => {
    ...
    expect(() => deploymentSettings("ethereum", config())).to.throw("Use bsc or bscTestnet");
